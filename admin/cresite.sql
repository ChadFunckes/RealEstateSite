...
--
-- Host: 127.0.0.1
-- Generation Time: Jul 04, 2016 at 04:50 PM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `checkEmail` (IN `emailin` TEXT)  BEGIN
SELECT email 
FROM users
WHERE email = emailin
LIMIT 1;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `checkUName` (`Uname` TEXT)  BEGIN
SELECT userName FROM users
WHERE userName = Uname
LIMIT 1;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `getUser` (`usr` TEXT, `pass` TEXT)  BEGIN
SELECT * 
FROM users
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `newUser` (IN `fname` TEXT, IN `lname` TEXT, IN `uname` TEXT, IN `uemail` TEXT, IN `upass` TEXT)  BEGIN
INSERT INTO users
VALUES (fname, lname, uname, uemail, upass, "a", "a", null, null, null, null, null, null, null,'');
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `putTempCode` (`email` TEXT, `code` INT)  BEGIN INSERT INTO templogin VALUES(email, code); 
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `updateUser` (IN `id` INT, IN `firstname` VARCHAR(15), IN `lastname` VARCHAR(15), IN `email` VARCHAR(45), IN `username` VARCHAR(15), IN `bio` TEXT, IN `address` TEXT, IN `job` TEXT, IN `skype_id` VARCHAR(45), IN `fone` INT(11), IN `role` VARCHAR(255), IN `faxnum` INT(13))  BEGIN
UPDATE users SET
    fname = firstname,
    lname = lastname,
    email = email,
...
CREATE TABLE `corporate_investors` (
...
CREATE TABLE `interest_proptypes` (
...
CREATE TABLE `lenders_mortgage_brokers` (
...
INSERT INTO `users` (`first`, `last`, `userName`, `office_address`, `user_role`, `fax`, `email`, `password`, `user_Type`, `subs_type`, `phone`, `bio`, `work_Hist`, `skype_ID`, `password_reset_token`, `password_reset_exp`, `activation_token`, `USER_ID`) VALUES
('jerry', 'curl', 'jcurl', '', '', 0, 'jcurl@test.com', '12345', 'C', 'a', NULL, NULL, NULL, NULL, NULL, NULL, NULL, 1),
('ramin', 'kamal', 'raminjan', '', '', 0, 'ramin@outlook.com', '12345', 'a', 'a', NULL, NULL, NULL, NULL, NULL, NULL, NULL, 2);
